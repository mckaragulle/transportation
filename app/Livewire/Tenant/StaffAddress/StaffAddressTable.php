...
final class StaffAddressTable extends PowerGridComponent
{
    use WithExport;
    ...
    public string $tableName = 'StaffAddressTable';
    ...
    public function setUp(): array
    {
        ...
        return [
            PowerGrid::cache()
                ->ttl(60)
                ->prefix($id . '_'),
            PowerGrid::exportable(fileName: 'Personel Adresleeri')
                ->striped()
    ...
    {
        $staff = StaffAddress::query()->whereStaffId($this->staff_id);
        return $staff;
    }
    ...
    public function columns(): array
    {
        return [
            Column::make('Id', 'id')
                ->sortable()
                ->searchable(),
            Column::make('Personel Adı', 'staff_id')
                ->sortable()
                ->searchable(),
            ...
            Column::make('Adres Başlığı', 'name')
                ...
                ->searchable()
                ->editOnClick(
                    hasPermission: auth()->user()->can('update staff_addresses'),
                    fallback: '- empty -'
                ),
    ...
    public function actions(StaffAddress $row): array
    {
        return [
            Button::add('view')
                ->slot('<i class="fa fa-pencil"></i>')
                ->route('staff_addresses.edit', ['id' => $row->id])
                ->class('badge badge-info'),
            Button::add('delete')
                ->slot('<i class="fa fa-trash"></i>')
                ->id()
                ->class('badge badge-danger')
                ->dispatch('delete-staff_address', ['id' => $row->id]),
        ];
    }
    ...
    public function actionRules($row): array
    {
        return [
            Rule::button('view')
                ->when(fn($row) => auth()->user()->can('update staff_addresses') != 1)
                ->hide(),
            Rule::button('delete')
                ->when(fn($row) => auth()->user()->can('delete staff_addresses') != 1)
                ->hide(),
        ];
    }
    ...
    {
        StaffAddress::query()->find($id)->update([
            $field => e($value) ? 1 : 0,
        ]);
    }
