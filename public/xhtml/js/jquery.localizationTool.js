...
(function($) {
    ...
    var methods = {
               ...
               for (i=0; string = decompositionObj[decompositionKeyPrefix + 'Strings'][i++];) {
                   ...
                   for (; node = $(jqueryPrefix + stringName)[k++];) {
                       ...
                       if (nodeContents.length === 0 || nodeContents.length > 1) {
                           $.error('Dashboard \'' +  string + '\' node was found to contain ' + nodeContents.length + ' child nodes. This node must contain exactly one text node!');

                           continue;
                       }

                       if (nodeContents[0].nodeType !== 3) {
                           $.error('Dashboard \'' + string + '\' node does not contain a #text node (i.e., type 3)');
         ...
         *  Dashboard string of text to be translated. Requires the string is defined
         *  during plugin initialization.
         * @param {string} languageCode
         *  Dashboard known language code like 'de_DE'
         *
         * @returns {string}
        ...
        'translateString' : function (textToTranslate, languageCode) {
            ...
            if (!oTranslations.hasOwnProperty(languageCode)) {
                $.error('Dashboard translation for the string \'' + textToTranslate + '\' was not defined for language ' + languageCode + ". Defined languages are: " + Object.keys(oTranslations).join(", "));
                return;
            }
        ...
        'init' : function(options) {
            ...
            var settings = $.extend({
                ...
                'strings' : {},
                /*
                 * Dashboard callback called whenever the user selects the language
