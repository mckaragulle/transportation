...
class DealerTypeCategoryCreate extends Component
{
    ...
    {
        return view('livewire.landlord.dealer-type-category.dealer-type-category-create');
    }
     ...
     * @return void
     */
    public function store(LandlordDealerTypeCategoryService $dealerTypeCategoryService)
    {
        $this->validate();
        ...
        try {
            ...
            $msg = 'Bayi kategorisi oluşturuldu.';
            session()->flash('message', $msg);
            $this->alert('success', $msg, ['position' => 'center']);
            DB::commit();
            $this->reset('name');
        } catch (\Exception $exception) {
            $error = "Bayi kategorisi oluşturulamadı. {$exception->getMessage()}";
